...
contract ShiftDollarShare is ERC20Burnable, Operator {
    using SafeMath for uint256;

    // TOTAL MAX SUPPLY = 1.000.000 PShare
    uint256 public constant FARMING_POOL_REWARD_ALLOCATION = 500000 ether;
    uint256 public constant TREASURY_FUND_POOL_ALLOCATION = 175000 ether;
    uint256 public constant DAO_FUND_POOL_ALLOCATION = 200000 ether;
    uint256 public constant DEV_FUND_POOL_ALLOCATION = 125000 ether;
    ...
    constructor(uint256 _startTime, address _daoFund, address _devFund, address _treasuryFund) public ERC20("ShiftDollar Share", "sfShare") {
        _mint(msg.sender, 3001 ether); // mint 3000 SHARE for Lauchpad - 1 Share for init lp
    ...
    function setBurnRate(uint256 _burnRate) external onlyOwner {
        require(_burnRate <= 1000, "too high"); // <= 15%
        burnRate = _burnRate;
    }
    ...
    function _transfer(
        ...
        require(sender != address(0), "SFSHARE: transfer to the zero address");
        require(
            recipient != address(0),
            "SFSHARE: transfer to the zero address"
        );
